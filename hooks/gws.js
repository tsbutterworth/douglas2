...
class GwsClient {
  ...
  async post(body, sendCookie = true) {
    ...
    // The DLL returns JSON but does not always set a JSON content type, so
    // read as text and parse explicitly.
...
/**
 * fetch has no timeout option, and a GWS that stops responding mid-request
 * would otherwise hang the hook until the runtime kills it.
 */
