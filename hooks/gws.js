...
class GwsClient {
  ...
  constructor(config, logEvent, sdk) {
    ...
    this.sdk = sdk;
    this.log = bindLogger(logEvent, sdk);
  }
...
/**
 * `sdk.logEvent` is a prototype method that internally calls
 * `this._eventEmitter.emit(...)`. Passing it around as a bare reference
 * (`const log = sdk.logEvent`) loses `this`, and calling it then throws
 * "Cannot read properties of undefined (reading 'emit')" — which looks
 * nothing like a logging problem and is easy to misattribute to whatever
 * operation happened just before it.
 *
 * Always bind it to the sdk. Logging is also wrapped so that a logging
 * failure can never take down real work.
 */
function bindLogger(logEvent, sdk) {
  const fn =
    sdk && typeof sdk.logEvent === "function"
      ? sdk.logEvent.bind(sdk)
      : typeof logEvent === "function" && sdk
      ? logEvent.bind(sdk)
      : logEvent;
  ...
  return (msg) => {
    try {
      ...
      /* never let logging break the driver */
    }
  };
}
...
function getClient(config, logEvent, sdk) {
  ...
  if (!_clients[key]) {
    ...
    _clients[key].log = bindLogger(logEvent, sdk);
